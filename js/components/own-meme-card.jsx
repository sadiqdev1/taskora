...
import { Bookmark, Copy, Edit2, Eye, EyeOff, Flag, Loader2, Maximize2, MessageCircle, MoreHorizontal, Share2, Trash2, X } from 'lucide-react';
import { useEffect, useRef, useState } from 'react';
import { Link, router, usePage } from '@inertiajs/react';
import { Avatar, CommentInput } from './comment-item';
...
function OwnTitleText({ authorName, isVerified, title, slug }) {
...
export default function OwnMemeCard({ meme, onDelete }) {
  const { auth } = usePage().props;

  const [reactionType, setReactionType] = useState(meme.reaction_type ?? null);
  ...
  useEffect(() => {
    const h = (e) => { if (e.key === "Escape") { setCommentOpen(false); setLightboxOpen(false); setMenuOpen(false); setDeleteModalOpen(false); } };
  ...
  const openComments = async () => {
    if (!auth?.user) {
      setAuthPromptAction('comment on memes');
  ...
  const handleEdit = () => {
    router.visit(`/edit/${meme.slug}`);
  };

  const handleDelete = async () => {
    setDeleting(true);
    try {
      await axios.delete(`/p/${meme.slug}`);
      onDelete?.(meme.id);
  ...
  return (
    <>
      <article className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl overflow-hidden hover:shadow-md dark:hover:shadow-zinc-900 transition-shadow duration-200 group">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="relative flex-shrink-0" ref={menuRef}>
            <button onClick={() => setMenuOpen((p) => !p)} type="button" className="p-2 rounded-full text-gray-400 dark:text-zinc-500 hover:text-gray-600 dark:hover:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-all"><MoreHorizontal size={17} /></button>
            {menuOpen && (
              <div className="absolute right-0 mt-1.5 w-52 bg-white dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700 rounded-xl shadow-xl py-1.5 z-20" style={{ animation: "menuIn .15s ease" }}>
                <button onClick={handleEdit} type="button" className="w-full text-left px-4 py-2.5 text-sm flex items-center gap-3 transition-colors text-gray-700 dark:text-zinc-200 hover:bg-gray-50 dark:hover:bg-zinc-700">
                  <Edit2 size={15} className="text-gray-400 dark:text-zinc-500 flex-shrink-0" />Edit
                </button>
                <button onClick={handleSave} type="button" className="w-full text-left px-4 py-2.5 text-sm flex items-center gap-3 transition-colors text-gray-700 dark:text-zinc-200 hover:bg-gray-50 dark:hover:bg-zinc-700">
                  <Bookmark size={15} className="text-gray-400 dark:text-zinc-500 flex-shrink-0" />{saved ? "Saved ✓" : "Save"}
                </button>
                <button onClick={handleShare} type="button" className="w-full text-left px-4 py-2.5 text-sm flex items-center gap-3 transition-colors text-gray-700 dark:text-zinc-200 hover:bg-gray-50 dark:hover:bg-zinc-700">
                  <Copy size={15} className="text-gray-400 dark:text-zinc-500 flex-shrink-0" />Copy link
                </button>
                <button onClick={() => setMenuOpen(false)} type="button" className="w-full text-left px-4 py-2.5 text-sm flex items-center gap-3 transition-colors text-gray-700 dark:text-zinc-200 hover:bg-gray-50 dark:hover:bg-zinc-700">
                  <EyeOff size={15} className="text-gray-400 dark:text-zinc-500 flex-shrink-0" />Hide post
                </button>
                <button onClick={() => { setMenuOpen(false); setDeleteModalOpen(true); }} type="button" className="w-full text-left px-4 py-2.5 text-sm flex items-center gap-3 transition-colors text-gray-700 dark:text-zinc-200 hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-600">
                  <Trash2 size={15} className="text-gray-400 dark:text-zinc-500 flex-shrink-0" />Delete
                </button>
              </div>
            )}
          </div>
        ...
        <div className="relative">
          ...
          <Link href={`/p/${meme.slug}`} className="block bg-gray-100 dark:bg-zinc-800 overflow-hidden">
            <img src={meme.image_url} alt={meme.title} className="w-full object-contain max-h-[600px]" loading="lazy" />
          </Link>
          )}
          <button onClick={() => setLightboxOpen(true)} type="button" className="absolute top-2 right-2 p-1.5 rounded-lg bg-black/30 hover:bg-black/50 text-white opacity-0 group-hover:opacity-100 transition-all backdrop-blur-sm"><Maximize2 size={14} /></button>
        </div>
        ...
        <div className="px-4 pt-3 pb-1">
          <div className="flex items-center gap-3 text-xs text-gray-400 dark:text-zinc-500 mb-1.5">
            ...
            <button onClick={openComments} type="button" className="flex items-center gap-1 hover:text-gray-600 dark:hover:text-zinc-300 transition-colors"><MessageCircle size={13} />{meme.comments_count ?? 0} comments</button>
            <span className="flex items-center gap-1"><Eye size={13} />{fmt(meme.views_count ?? 0)} views</span>
          </div>
          <OwnTitleText authorName={authorName} isVerified={meme.user?.is_verified} title={meme.title} slug={meme.slug} />
          {meme.tags?.length > 0 && (
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm md:flex md:items-center md:justify-center md:p-4" role="dialog" aria-modal="true" onClick={() => setCommentOpen(false)}>
          <div ref={sheetRef} className="absolute md:relative bottom-0 md:bottom-auto left-0 md:left-auto right-0 md:right-auto bg-white dark:bg-zinc-900 rounded-t-3xl md:rounded-2xl shadow-2xl border-t md:border border-gray-100 dark:border-zinc-800 flex flex-col w-full md:max-w-2xl" style={{ height: '85vh', maxHeight: '85vh', animation: "slideUp .3s cubic-bezier(.22,.61,.36,1)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 dark:border-zinc-800">
              <div><h3 className="text-base font-semibold text-gray-900 dark:text-white">Comments</h3><p className="text-xs text-gray-400 dark:text-zinc-500">{meme.comments_count ?? 0} comment{(meme.comments_count ?? 0) !== 1 ? "s" : ""}</p></div>
              <button onClick={() => setCommentOpen(false)} type="button" className="p-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-full transition" aria-label="Close comments">
      ...
      {lightboxOpen && meme.post_type !== 'text' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 p-4" onClick={() => setLightboxOpen(false)} role="dialog" aria-modal="true">
          <div className="relative max-w-4xl max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setLightboxOpen(false)} type="button" className="absolute -top-3 -right-3 p-2 rounded-full bg-white/10 hover:bg-white/20 text-white transition backdrop-blur-sm z-10"><X size={18} /></button>
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto" onClick={() => !deleting && setDeleteModalOpen(false)}>
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-md border border-gray-100 dark:border-zinc-800 my-8" style={{ animation: "modalIn .25s cubic-bezier(.16,1,.3,1)" }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-zinc-800">
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">Delete Meme</h3>
              <button onClick={() => setDeleteModalOpen(false)} disabled={deleting} className="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 dark:text-zinc-500 hover:bg-gray-100 dark:hover:bg-zinc-800 transition disabled:opacity-50">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="px-6 py-5">
              <div className="flex gap-3 mb-4">
                ...
                <div>
                  <p className="text-sm text-gray-700 dark:text-zinc-300 font-medium mb-1">Are you sure you want to delete this meme?</p>
                  <p className="text-xs text-gray-500 dark:text-zinc-400">This action cannot be undone. All likes, comments, and shares will be permanently removed.</p>
                </div>
              </div>

              <div className="bg-gray-50 dark:bg-zinc-800 rounded-xl p-3 mb-4">
                <p className="text-xs font-semibold text-gray-900 dark:text-white truncate">{meme.title}</p>
                <div className="flex items-center gap-3 mt-1 text-xs text-gray-400 dark:text-zinc-500">
                  <span>{fmt(likesCount)} likes</span>
                  <span>•</span>
                  <span>{meme.comments_count ?? 0} comments</span>
                  <span>•</span>
                  <span>{fmt(meme.views_count ?? 0)} views</span>
                </div>
              </div>

              <div className="flex justify-end gap-2.5">
                <button onClick={() => setDeleteModalOpen(false)} disabled={deleting} type="button" className="px-4 py-2 border border-gray-200 dark:border-zinc-700 rounded-full text-sm font-medium text-gray-600 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-800 transition disabled:opacity-50">
                  Cancel
                </button>
                <button onClick={handleDelete} disabled={deleting} type="button" className="px-5 py-2 bg-red-500 hover:bg-red-600 text-white rounded-full text-sm font-semibold transition-all flex items-center gap-2 disabled:opacity-60">
                  {deleting ? (
                    <>
