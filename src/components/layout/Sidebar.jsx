...
const Sidebar = () => {
  ...
  return (
    <aside className="hidden md:fixed md:inset-y-0 md:flex md:w-64 md:flex-col z-20 bg-white dark:bg-zinc-900 border-r border-gray-200 dark:border-zinc-800">
      <div className="flex flex-col h-full">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
          ...
          {(user?.role === 'admin' || user?.role === 'moderator') && (
            <>
              ...
              >
                <span className={`text-[17px] flex-shrink-0 transition-colors ${
                  location.pathname.startsWith('/admin') ? 'text-blue-500' : 'text-gray-500 dark:text-zinc-500 group-hover:text-gray-700 dark:group-hover:text-zinc-300'
                }`}>
                  <IoGrid />
                </span>
        ...
        <div className="border-t border-gray-200 dark:border-zinc-800 p-3">
          <div className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors">
            <img
              src={user?.avatar}
              alt={user?.name}
