    ...
    <DashboardLayout title="My Tasks" subtitle="Track all your task submissions">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
        {[
          ...
          { label: 'Total Earned', value: '₦' + totalEarned.toLocaleString('en-NG', { minimumFractionDigits: 2 }), Icon: DollarSign, bg: '#EEF2FF', color: 'var(--primary)' },
        ].map(s => (
                ...
                <tr key={s.id}
                  ...
                  <td className="px-5 py-3.5">
                    <span className="font-bold text-sm" style={{ color: s.status === 'approved' ? '#00875A' : 'var(--text-muted)' }}>
                      {s.status === 'approved'
                        ? '+₦' + parseFloat(s.earned || 0).toLocaleString('en-NG', { minimumFractionDigits: 2 })
                  ...
                  <td className="px-5 py-3.5">
                    {s.status === 'rejected' && (
                      <Link href={`/tasks/${s.campaign?.slug}`} className="text-xs font-semibold" style={{ color: 'var(--primary)' }}>
                        Retry
                      </Link>
                    )}                  </td>
                </tr>
